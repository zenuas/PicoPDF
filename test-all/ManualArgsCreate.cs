...
public class ManualArgsCreate : FontRegisterCommand
{
    ...
    [CommandOption("font-option")]
    public FontLoadOptions FontOption { get; init; } = FontLoadOptions.PossibleEmbed;
    ...
    public override void Run(string[] args)
    {
        ...
        var font = Type0Font.Create("fo", fontreg.LoadFont(Font), FontOption);
        document.Resources.Fonts.Add(font);
        foreach (var arg in args)
        {
            var page = document.NewPage(width, height);
            page.Contents.Operations.Add(DrawString.Create(arg, Left, Top, Point, [font], document, width - Left, style: TextStyles.MultiLine));
        }
