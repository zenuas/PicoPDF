...
if (opt.RegisterUserFont != "" && !Directory.Exists(opt.RegisterUserFont)) _ = fontreg.LoadComplete(opt.RegisterUserFont);
if (opt.RegisterUserFont != "" && Directory.Exists(opt.RegisterUserFont)) fontreg.RegisterDirectory(new OpenType.LoadOption() { ForceEmbedded = true }, opt.RegisterUserFont);

if (opt.FontFileExtract != "") { FontFileExport.Export(fontreg.LoadComplete(opt.FontFileExtract), opt); return; }
...
if (opt.FontDump != "") { FontDump.Dump(fontreg.LoadComplete(opt.FontDump), opt); return; }
