...
public class AxialShading : PdfObject, IShading
{
    ...
    public override void DoExport(PdfExportOption option)
    {
        ...
        _ = Elements.TryAdd("Coords", $"[{Coords.X0.ToPoint().PointToString(option.PointFormat)} {Coords.Y0.ToPoint().PointToString(option.PointFormat)} {Coords.X1.ToPoint().PointToString(option.PointFormat)} {Coords.Y1.ToPoint().PointToString(option.PointFormat)}]");
