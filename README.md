...
PicoPDF.Pdf.Documents.Document.CreateDocument("example.json", datas).Save("example.pdf");
```
...
カラーグリフ抽出にはCOLR、CPAL形式にのみ対応している。CBDT、CBLC、sbix、SVGなどには対応していない。  
RTLや縦書きには対応していない。  
しおりやリンクには対応していない。  
PDFのAes128暗号化に対応している。  
