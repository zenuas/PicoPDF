...
public static class SectionBinder
{
        ...
        where M : ISectionModel<M>
    {
        ...
        if (datas.IsLast)
        {
            ...
            bind.SetPageCount(1);
            headers.Select(x => M.CreateSectionModel(page, x.Section, left, nodata, bind, x.BreakCount, x.Depth)).Each(models.Add);
            footers.FooterSort().Select(x => M.CreateSectionModel(page, x.Section, left, nodata, bind, x.BreakCount, x.Depth).Return(x => bind.BreakSection(x))).Each(models.Add);
            if (page.Footer is ISection lastfooter) models.Add(M.CreateSectionModel(page, lastfooter, left, nodata, bind, 0, null).Return(x => bind.BreakSection(x)));
        ...
        while (!datas.IsLast)
        {
            ...
            headers
                ...
                .Select(x => M.CreateSectionModel(page, x.Section, left, firstdata, bind, x.BreakCount, x.Depth))
                .Each(models.Add);
            var page_first = true;
            var breakcount = 0;
            ...
            while (true)
            {
                ...
                if (count == 0)
                {
                    if (everyfooter is { }) models.Add(M.CreateSectionModel(page, everyfooter, left, lastdata, bind, 0, null).Return(x => bind.BreakSection(x)));
                    break;
                }
                ...
                breakheader?.Select(x => M.CreateSectionModel(page, x.Section, left, current, bind, x.BreakCount, x.Depth)).Each(models.Add);

                _ = datas.Next(count - 1, out lastdata);
                foreach (var x in datas.GetRange(count))
                {
                    bind.DataBind(x);
                    models.Add(M.CreateSectionModel(page, detail, left, x, bind, keys.Length, null));
                }
                lastdetail = models.Last();
                breakfooter.FooterSort().Select(x => M.CreateSectionModel(page, x.Section, left, lastdata, bind, x.BreakCount, x.Depth).Return(x => bind.BreakSection(x))).Each(models.Add);
                ...
                if (datas.IsLast)
                {
                    if (page.Footer is ISection lastfooter) models.Add(M.CreateSectionModel(page, lastfooter, left, lastdata, bind, 0, null));
                    break;
                }
            }
