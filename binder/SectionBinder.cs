...
public static class SectionBinder
{
        ...
        where TSection : ISectionModel<TSection>
    {
        ...
        var break_section = ((Action<TSection>)bind.BreakSection).Return();
        ...
        if (datas.IsLast)
        {
            ...
            bind.SetPageCount(1);
            headers
                .Select(x => TSection.CreateSectionModel(page, x.Section, nodata, bind, x.BreakCount, x.Depth))
                .Each(models.Add);
            footers
                .FooterSort()
                .Select(x => TSection.CreateSectionModel(page, x.Section, nodata, bind, x.BreakCount, x.Depth))
                .Select(break_section)
                .Each(models.Add);
            if (page.Footer is ISection lastfooter) models.Add(break_section(TSection.CreateSectionModel(page, lastfooter, nodata, bind, 0, null)));
        ...
        while (!datas.IsLast)
        {
            ...
            while (true)
            {
                ...
                var pagelast_breakfooter_height = models.Where(x => x.ViewMode == ViewModes.PageLast).Select(x => x.Height).Sum();

                var temp_everyfooter = everyfooter is null || page_first ? default : TSection.CreateSectionModel(page, everyfooter, lastdata, bind, 0, null);
                if (!page_first) bind.SectionBreak(lastdata, page);

                var details = GetBreakOrTakeDetail(page, detail, datas, bind, keys, height - minimum_breakfooter_height + pagelast_breakfooter_height);
                if (details.Count == 0)
                {
                    if (temp_everyfooter is { } && temp_everyfooter.IsVisible) models.Add(break_section(temp_everyfooter));
                    break;
                }
                ...
                var breakfooter_section = breakfooter
                    .FooterSort()
                    .Select(x => TSection.CreateSectionModel(page, x.Section, lastdata, bind, x.BreakCount, x.Depth))
                    .Select(break_section)
                    .ToArray();

                if (breakcount > 0 && (detail.IsFill || breakfooter_section.Any(x => x.IsFill)))
                {
                    var fillarea = height - details.Select(x => x.Section.Height).Sum() - breakfooter_section.Where(x => x.IsVisible).Select(x => x.Height).Sum();
                    while (fillarea > 0)
                    {
                        var fill = TSection.CreateSectionModel(page, detail, default!, bind.Empty, keys.Length, null);
                ...
                if (breakfooter_section.Contains(x => x.IsPageBreak))
                {
                    if (everyfooter is { })
                    {
                        var pagebreak_footer = TSection.CreateSectionModel(page, everyfooter, lastdata, bind, 0, null);
                        if (pagebreak_footer.IsVisible) models.Add(break_section(pagebreak_footer));
                    }
                ...
                {
                    var pagelast_footer = TSection.CreateSectionModel(page, lastfooter, lastdata, bind, 0, null);
        ...
        where TSection : ISectionModel<TSection>
    {
        ...
        bind.DataBind(prev);
        var first = TSection.CreateSectionModel(page, detail, prev, bind, 0, null);
        ...
        while (true)
        {
            ...
            bind.DataBind(data);
            var next = TSection.CreateSectionModel(page, detail, data, bind, 0, null);
            height += next.Height;
    ...
    public static IEnumerable<SectionInfo> SkipWhileOrEveryPage(this SectionInfo[] self, Func<SectionInfo, bool> f)
    {
        ...
        {
            if (!found && f(x)) found = true;
            if (found || x.Section.ViewMode == ViewModes.Every || x.Section.ViewMode == ViewModes.PageLast)
            {
                yield return x;
            }
        }
