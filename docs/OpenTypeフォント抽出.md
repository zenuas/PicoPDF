...
下記はフォントファイルから「abc」の3文字のみを抽出したサンプルである。

* [TTF形式フォントから抽出、Noto Sans JP](sample/subset-ttf.ttf){:target="_blank"}
* [CFF形式フォントから抽出、Noto Sans CJK JP](sample/subset-cff.otf){:target="_blank"}

これらは元フォントから読み込みから抽出の過程で改変が加えられている。

TTF形式は[Simple glyph](TrueTypeラスタライズ)の情報が再配置されている。
CFF形式は[CharStrings](CFFラスタライズ)のステム削除、rmoveto、rlineto、rrcurveto命令のみで再構築が行われている。
オリジナルフォントは[Noto - Google Fonts](https://fonts.google.com/noto)から入手すること。
