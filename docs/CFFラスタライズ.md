...
| 12 25      | 予約                                             |                                                                                                                                                        |
| 12 26      | [sqrt](#not、neg、abs、sqrt)                     | num sqrt                                                                                                                                               |
...
スタックの先頭インデックスのローカルサブルーチンかグローバルサブルーチンを呼び出す。
スタックを引数にする。
サブルーチンの個数でバイアスを決め、スタック先頭インデックスにバイアスを足す。

| サブルーチン個数        | バイアス |
|-------------------------|---------:|
| 1240個未満              |      107 |
| 1241個以上、33900個未満 |     1131 |
| 33901個以上             |    32768 |

```cs
var bias =
    subr.Length < 1240 ? 107 :
    subr.Length < 33900 ? 1131 :
    32768;
var index = stack.Pop() + bias;
// subr[index] へジャンプ
```
...
switch (ope)
{
	case Not: stack.Push(stack.Pop() != 0 ? 0 : 1); break;
