    ...
    public static Surface CreateSurface(int start, int end, SimpleGlyphFlags[] flags, int[] xcoordinates, int[] ycoordinates, float xmin, float ymin, float xmax, float ymax)
    {
        ...
        for (var i = start + 1; i <= end; i++)
        {
            ...
            else if (i + 1 <= end)
            {
                ...
                {
                    edges.Add(new BezierCurve { Start = prev, End = next2, ControlPoint = [next], ComplementPoint = false });
                    prev = next2;
                    i++;
                }
                else
                {
                    ...
                    edges.Add(new BezierCurve { Start = prev, End = complement_point, ControlPoint = [next], ComplementPoint = true });
