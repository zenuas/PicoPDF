                ...
                case LinearGradientLayer linear when linear.StopColors.Length > 0:
                    {
                        ...
                        {
                            var shading = AxialShading.Create($"sh{document.Shadings.Count}", linear, ColorToAlpha, "/DeviceGray").Return(document.Shadings.Add);
                            ...
                            gstream.Flush();
                            opes.Add(new DrawPathExtGState { ExtGState = GraphicsStateParameter.CreateTransparency($"gs{document.GraphicsStateParameters.Count}", g).Return(document.GraphicsStateParameters.Add) });
                        }
                        opes.Add(new DrawPathShading { Shading = AxialShading.Create($"sh{document.Shadings.Count}", linear, ColorToRGB, "/DeviceRGB").Return(document.Shadings.Add) });
                        break;
                    }
                ...
                case RadialGradientLayer radial when radial.StopColors.Length > 0:
                    {
                        ...
                        {
                            var shading = RadialShading.Create($"sh{document.Shadings.Count}", radial, ColorToAlpha, "/DeviceGray").Return(document.Shadings.Add);
                            // Set a sufficiently large enough for the BBox.
                        ...
                        opes.Add(new DrawPathShading { Shading = RadialShading.Create($"sh{document.Shadings.Count}", radial, ColorToRGB, "/DeviceRGB").Return(document.Shadings.Add) });
                        break;
                    }
