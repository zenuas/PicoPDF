...
public class Aes256Handler : ISecurityHandler
{
    ...
    public static (PdfObject Encrypt, byte[] EncryptionKey) Create(CFM cfm, string user_password, string owner_password, UserAccessPermissions permissions)
    {
        ...
        var file_encryption_key = CreateFileEncryptionKey();
        Span<byte> u_key = stackalloc byte[48];
        Span<byte> ue_key = stackalloc byte[32];
        ComputeUserPassword_Algorithm8(user_password_bytes.Length > 127 ? user_password_bytes[..127] : user_password_bytes, file_encryption_key, u_key, ue_key);

        Span<byte> o_key = stackalloc byte[48];
        Span<byte> oe_key = stackalloc byte[32];
        ComputeOwnerPassword_Algorithm9(owner_password_bytes.Length > 127 ? owner_password_bytes[..127] : owner_password_bytes, file_encryption_key, u_key, o_key, oe_key);

        Span<byte> perms = stackalloc byte[16];
        ComputePerms_Algorithm10(permissions, true, file_encryption_key, perms);
